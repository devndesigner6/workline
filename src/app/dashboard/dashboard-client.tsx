...
export function DashboardClient() {
  ...
  // hydration mismatch); localStorage is unavailable on the server. Deferred
  // a tick so the external read never sets state synchronously in the effect.
  const [tasks, setTasks] = useState<BuyerTaskRecord[] | null>(null);
  useEffect(() => {
    const timer = setTimeout(() => setTasks(getBuyerTaskRecords()), 0);
